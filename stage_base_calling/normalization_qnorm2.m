...
puncta_set_normalized = puncta_set_cell;
...
%color into a vector
punctaindices_vecpos = zeros(N*readlength,1);
data_cols = zeros(total_voxels*readlength,4);
pos_cur = 1;
for rnd_idx = 1:readlength
    ...
    for p_idx = 1:N
        ...
        punctaindices_vecpos(p_idx+(rnd_idx-1)*N) = pos_next;
        pos_cur = pos_next+1;     
    end
...
toc
fprintf('Completed quantile normalization in both dimensions\n');
%
%Unpack the normalized colors back into the original shape
pos_cur = 1;
for rnd_idx = 1:readlength
    for p_idx = 1:N
        puncta_set_normalized{rnd_idx}{p_idx,1} = data_cols_norm(pos_cur:punctaindices_vecpos(p_idx+(rnd_idx-1)*N),1);
        puncta_set_normalized{rnd_idx}{p_idx,2} = data_cols_norm(pos_cur:punctaindices_vecpos(p_idx+(rnd_idx-1)*N),2);
        puncta_set_normalized{rnd_idx}{p_idx,3} = data_cols_norm(pos_cur:punctaindices_vecpos(p_idx+(rnd_idx-1)*N),3);
        puncta_set_normalized{rnd_idx}{p_idx,4} = data_cols_norm(pos_cur:punctaindices_vecpos(p_idx+(rnd_idx-1)*N),4);
        %Track the 1D index as we unpack out each puncta
        pos_cur = punctaindices_vecpos(p_idx+(rnd_idx-1)*N)+1;
    end
