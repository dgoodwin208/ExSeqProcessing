...
imgR = load3DTif_uint16(fullfile(DIRECTORY,sprintf('%s_round%.03i_ch0%i.%s',FILEROOT_NAME,roundnum,1-1,params.IMAGE_EXT)));


chan_offsets = zeros(4,3);
for c = 2:params.NUM_CHANNELS
	imgM = load3DTif_uint16(fullfile(DIRECTORY,sprintf('%s_round%.03i_ch0%i.%s',FILEROOT_NAME,roundnum,c-1,params.IMAGE_EXT)));
	...
	save3DTif_uint16(imgM_shift,fullfile(OUTPUTDIR,sprintf('%s_round%0.3i_ch%0.2iSHIFT.%s',FILEROOT_NAME,roundnum,c-1,params.IMAGE_EXT)));
end
