function performAffineTransforms_global(fixed_fov, moving_fovmatches, moving_round,do_downsample,bigparams)
...
for fov_mov = moving_fovmatches
    ...
    try
        ...
        [affine_tform,~]  = getGlobalAffineFromCorrespondences(keyM_total,keyF_total);
    catch
...
%     maxNumCompThreads(worker_max_threads);
    ...
    for c = 1:length(ch_list)
        ...
        filename = fullfile(inputdir,sprintf('%s_round%03d_%s.%s',filename_root,moving_round,data_channel,image_ext));
