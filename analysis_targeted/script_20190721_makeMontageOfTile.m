...
%Experiment: bc each tile is transposed, transpose the tile map!
tileMap_indices = fliplr(tileMap_indices);
...
parfor row = 1:size(tileMap_indices,1)
    ...
    for col = 1:size(tileMap_indices,2)
        ...
        all_mips{row}{col} = mip; %DON'T Undo a transpose that seems to happen in one of the conversions
...
for row = 1:size(tileMap_indices,1)
    for col = 1:size(tileMap_indices,2)
        ...
        %complete_stitch(1+imgSizeXY(2)*(col-1):imgSizeXY(2)*col,...
        %    1+imgSizeXY(1)*(row-1):imgSizeXY(1)*row) = all_mips{row}{col};
    end
end
