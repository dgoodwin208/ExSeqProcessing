function [max_run_jobs,max_run_jobs_downsampled] = concurrency_size_in_normalization_cuda(cond)

    loadParameters;

    if ~isfield(params,'NORM_MAX_RUN_JOBS')
        main_mem_usage = params.MATLAB_PROC_CONTEXT;
        job_worker_mem_usage = 4 * cond.imgsize_dbl + params.MATLAB_PROC_CONTEXT;
    ...
    if ~isfield(params,'NORM_DOWNSAMPLE_MAX_RUN_JOBS')
        main_mem_usage = params.MATLAB_PROC_CONTEXT;
        job_worker_mem_usage = 4 * cond.downsample_imgsize_dbl + params.MATLAB_PROC_CONTEXT;
