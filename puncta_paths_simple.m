...
mask_img = zeros(size(sum_img));
for p_idx = 1:length(filtered_puncta)
     mask_img(filtered_puncta(p_idx).PixelIdxList)=100;
end

filename_out = fullfile(params.punctaSubvolumeDir,sprintf('%s_TotalPunctaMask.tif',params.FILE_BASENAME));
save3DTif_uint16(mask_img,filename_out);
