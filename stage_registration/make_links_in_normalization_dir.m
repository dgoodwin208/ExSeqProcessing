function ret = make_links_in_normalization_dir()
    ...
    for do_downsample = [true,false]
        ...
        for r_i = 1:params.NUM_ROUNDS
            for c_i = 1:length(params.CHAN_STRS)
                src_filename = fullfile(src_dir,sprintf('%s_round%03i_%s.%s',FILEROOT_NAME,r_i,params.CHAN_STRS{c_i},params.IMAGE_EXT));
                if ~exist(src_filename,'file')
                    fprintf('[ERROR] not exist source file: %s\n',src_filename);
                    ret = false;
                    continue
                end
                dst_filename = sprintf('./%s_round%03i_%s.%s',FILEROOT_NAME,r_i,params.CHAN_STRS{c_i},params.IMAGE_EXT);
