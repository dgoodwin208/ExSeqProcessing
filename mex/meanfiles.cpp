/*=================================================================
 * meanfiles.cpp - mean data files to one file
 *
 *  meanfiles(outputdir, meanfile_list, out_file)
 *
 *  outputdir(char):  directory to be stored out file
 *  meanfile_list(cell):  a list of triplets; (in1file, in2file, ..)
    ...
    while (1) {
        ...
        for (int i = 0; i < ifs.size(); i++) {
            ...
            for (int j = 0; j < read_buf_size / NUM_ELEMENTS; j++) {
                out_buf[j] += in_buf[j * NUM_ELEMENTS + IDX_MEAN];
            }
        }
        if (read_buf_size == 0) {
            break;
...
{ 
    ...
    meanFiles(outputdir, inpostfixes, outpostfix);
