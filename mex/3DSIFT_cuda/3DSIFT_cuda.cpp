                 ...
                 int nrhs, mxArray const *prhs[])
{
    ...
    if (nrhs < 2) {
        mexErrMsgIdAndTxt( "3DSIFT_cuda:InvalidInput","Two input arguments required.");
    } 
    if (nlhs > 1){
        mexErrMsgIdAndTxt( "3DSIFT_cuda:InvalidOutput","Only 0 or 1 output arguments.");
    }
    ...
    if ( !mxIsSingle(prhs[0])) {
        mexErrMsgIdAndTxt("3DSIFT_cuda:InvalidInput","Input array must be type single (float).");
    }
